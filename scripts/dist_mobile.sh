...
docker run --rm  \
    ...
    -v "$(pwd)":/workspace/go/src/github.com/Kdag-K/kdag \
    -w /workspace/go/src/github.com/Kdag-K/kdag \
    kdagworks/mobile:0.0.2 ./scripts/dist_mobile_build.sh
...
shasum -a256 ./* > "./kdag_${VERSION}_SHA256SUMS"
echo "$VERSION" > ./git.version
ZIP="./kdag_${VERSION}_android_library.zip"
zip "$ZIP" ./*  -x "$ZIP"
popd
