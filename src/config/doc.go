// Package config defines the configuration for a Kdag node.
//
// Regardless of how Kdag is started, directly from Go code or as a standalone
...
// configuration options, Kdag relies on a data directory, defined by
// Config.DataDir, where it expects to find a few additional configuration
// files:
//
//  priv_key // a plain text file containing the raw private key (cf. kdag keygen).
