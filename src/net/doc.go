// Package net implements different transports to communicate between Kdag
// nodes.
//
// This package contains various implementations of the Transport interface,
// which is used by Kdag nodes to send and receive RPC requests (SyncRequest,
...
// issues.
//
// To use a TCP transport, set the following configuration options in the Kdag
// Config object (cf config package):
//
// - BinAdddr: the IP:PORT of the TCP socket that Kdag binds to.
//
...
// WebRTC
//
// Because Kdag is a peer-to-peer application, it can run into issues with
...
//
// - WebRTC: tells Kdag to use a WebRTC transport
//
...
// when the WebRTC transport is used, Kdag is not fully p2p anymore. That
