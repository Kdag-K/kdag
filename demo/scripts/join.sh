...
# start the new node
docker run -d --name=client$N --net=kdagnet --ip=172.77.10.$N -it Kdag-K/dummy:latest \
    --name="client $N" \
...
docker create --name=node$N --net=kdagnet --ip=172.77.5.$N Kdag-K/kdag:latest run \
    --heartbeat=100ms \
    --moniker="node$N" \
    --cache-size=50000 \
